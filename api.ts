...
const setHostConnection = (newConnection: any) => {
...
const fetchConnect = async (): Promise<I_ConnectResponse | null> => {
  ...
  try {
    ...
    if (!res.ok)
      throw new Error(`[homebrew] HTTP error! Status: ${res.status}`);
...
// @TODO This needs to be put in Obrew Studio frontend only, it is the only consumer of this func
...
const createServices = (response: I_API[] | null): I_ServiceApis | null => {
  ...
  // Inject non-backend related methods. @TODO This should be in Obrew Studio project only.
